...
public class MoveGenerator {
    ...
    /*
     * Pawns go forward once, unless the way is blocked or it is their first
     * move and they go forward twice. They can only capture diagonally.
     */
    public void pawnMoves(Piece pawn, Board _curBoard) {
        ...
        // check for captures
        if (fileLeft >= 0) {
            if (_curBoard.getPieceAt(fileLeft, oneUp).getType().equals("null")) {
                fileLeft = -1;
            }
        }
        if (fileRight < Board.NUM_FILES) {
            if (_curBoard.getPieceAt(fileRight, oneUp).getType().equals("null")) {
                fileRight = -1;
            }
        }
        if (fileLeft < 0)
        ...
        if (fileLeft != -1) {
            ...
            String end = temp.getPieceAt(fileLeft, oneUp).getSquare();
        ...
        if (fileRight != -1) {
            Board temp = new Board(_curBoard);
            String end = temp.getPieceAt(fileRight, oneUp).getSquare();
            if (temp.executeMove(start + end)) {
                posMoves.add(temp);
            }
        }
        ...
        // check moves forward;
        if (_curBoard.getPieceAt(file, oneUp).getType().equals("null")) {
            Board temp = new Board(_curBoard);
        ...
        if (((_curBoard.blackToMove() && pawn.getColor().equals("black")) && rank == 6)
                || ((_curBoard.whiteToMove() && pawn.getColor().equals("white")) && rank == 1)) {
