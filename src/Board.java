...
public class Board {
    ...
    /*
     * Two different board representations: the first is piece-centric, ie the
     * piecelist. The other is the board. The board and piece list should
     * obviously always match. Board will be rank-major order, ie. board = { a1,
     * b1, c1, d1, e1, f1, g1, h1, a2, b2, c2, d2, e2, f2, g2, h2, a3, b3, c3,
     * d3, e3, f3, g3, h3, a4, b4, c4, d4, e4, f4, g4, h4, a5, b5, c5, d5, e5,
     * f5, g5, h5, a6, b6, c6, d6, e6, f6, g6, h6, a7, b7, c7, d7, e7, f7, g7,
     * h7, a8, b8, c8, d8, e8, f8, g8, h8 } except that of course that board is
     * stacked upside down. So e2e4 is a move from board[12] to board[28];
     */
    ...
    public Board() {
        ...
        for (int piece = NUM_PIECES / 2; piece < (NUM_SQUARES - NUM_PIECES / 2); piece++) {
    ...
    /*
     * This function will check for move partial legality. Some illegal moves
     * will return false, to simplify move generation. False guarentees an
     * illegal move, while a return value of true does not guarentee that the
     * move is legal. Checks involve leaving the board, capturing a teammate.
     * Returns false if colorToMove is in check after move is completed.
     */
    ...
    /*
     * First check to make sure squares in question actually exit. Next we check
     * to make sure that if there is a capture, it is a different colored piece.
     */
    public boolean moveIsLegal(int s_file, int s_rank, int e_file, int e_rank) {

        if (s_file < 0 || s_file >= NUM_FILES || s_rank < 0
                || s_rank >= NUM_RANKS || e_file < 0 || e_file >= NUM_FILES
    ...
    public boolean colorToMoveInCheck(int startPos, int endPos) {
        ...
        for (int i = offset; i < offset + 16; i++) {
            ...
            /******* Partial Queen ******************/
                ...
                if (rank == k_rank)
                    r_inc = 0;
                for (f = file, r = rank; (f != k_file) && (r != k_rank); f += f_inc, r += r_inc) {
